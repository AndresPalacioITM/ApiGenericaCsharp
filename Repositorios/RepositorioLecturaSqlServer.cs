...
namespace ApiGenericaCsharp.Repositorios
{
    public class RepositorioLecturaSqlServer : IRepositorioLecturaTabla
    {
        ...
        private bool EsFechaSinHora(string valor){
            return valor.Length == 10 && valor.Count(c=> c == '-') == 2 && !valor.Contains("T") && !valor.Contains(":");
        }

        public async Task<IReadOnlyList<Dictionary<string,object?>>> ObtenerFilasAsync(string nombreTabla, string? esquema, int? limite)
        {
            ...
            {
                throw new InvalidOperationException($"Error SQL al consultar'{esquemaFinal}.{nombreTabla}':{ex.Message}",ex);
            }
            return resultados;
        }
        public async Task<IReadOnlyList<Dictionary<string, object?>>>ObtenerPorClaveAsync(string nombreTabla,string? esquema,string nombreClave, string valor){
            if (string.IsNullOrWhiteSpace(nombreTabla)) throw new ArgumentException("El nombre de la tabla no puede estar vacio.",nameof(nombreTabla));
            if (string.IsNullOrWhiteSpace(nombreClave)) throw new ArgumentException("El nombre de la clave no puede estar vacio.",nameof(nombreClave));
            if(string.IsNullOrWhiteSpace(valor)) throw new ArgumentException("El valor no puede estar vacio.",nameof(valor));
            ...
            try
            {
                ...
                bool esBusquedaFechaSoloEnDatetime = (tipoColumna == SqlDbType.DateTime || tipoColumna == SqlDbType.DateTime2)&& EsFechaSinHora(valor);
                string sql;
