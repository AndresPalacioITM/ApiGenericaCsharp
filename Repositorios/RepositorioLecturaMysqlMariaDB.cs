...
{
    public class RepositorioLecturaMysqlMariaDB : IRepositorioLecturaTabla
    {
        ...
        { 
            ...
            { 
                foreach (var campo in camposEncriptar.Split(',', StringSplitOptions.RemoveEmptyEntries | 
StringSplitOptions.TrimEntries)) 
            ...
            string sql = 
                $"INSERT INTO {esquemaFinal}`{nombreTabla}` ({string.Join(", ", nombres)}) VALUES ({string.Join(", ", 
parametros)})"; 
        ...
        { 
            ...
            { 
                ...
                { 
                    ...
                    ["tiempoEncendido"] = $"{(int)tiempoEncendido.TotalDays} días, {tiempoEncendido.Hours} horas,{tiempoEncendido.Minutes} minutos", ["tipoConexion"] = interpretacion,["explicacion"] = "Si 'horaInicio' es reciente (minutos u horas), probablemente es Docker. Si es antigua (días o semanas),probablemente es base de datos local."}; 
