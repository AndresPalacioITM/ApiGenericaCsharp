...
namespace ApiGenericaCsharp.Controllers
{
    ...
    {
        private readonly IServicioCrud _servicioCrud;  //Para la lógica de negocio
        private readonly ILogger<EntidadesController> _logger; // Para el logging
        private readonly IConfiguration _configuration; // Para accesp a configuración desde appsettings.json

        public EntidadesController(IServicioCrud servicioCrud, ILogger<EntidadesController> logger, IConfiguration configuration)
        {
            _servicioCrud = servicioCrud ?? throw new ArgumentNullException(nameof(servicioCrud),"IServicioCrud no fue inyectado correctamente. Verificar registro de servicios en Program.cs");
            _logger = logger ?? throw new ArgumentNullException(nameof(logger), "ILogger no fue inyectado correctamente. Problema en configutacion de loggin de ASP.NET Core");
            _configuration = configuration ?? throw new ArgumentNullException(nameof(configuration), "IConfiguracion no fue inyectado correctamente. Problema en configuracion base de ASP.NET Core");
        }
        //[AllowAnonymous]
        //[Authorize]
        [HttpGet]
        public async Task<IActionResult>ListarAsync(string tabla, [FromQuery]string? esquema, [FromQuery] int? limite)
        {
            try
            {
                _logger.LogInformation("INICIO consulta - Tabla: {Tabla}, Esquema: {Esquema}, Limite: {Limite}", tabla, esquema ?? "por defecto", limite?.ToString() ?? "por defecto");
                var filas = await _servicioCrud.ListarAsync(tabla, esquema, limite);
                 _logger.LogInformation("RESULTADO exitoso - Registros obtenidos: {Cantidad} de tabla {Tabla}", filas.Count, tabla);
                ...
                return Ok(new { tabla = tabla, esquema = esquema ?? "por defecto", limite = limite, total = filas.Count, datos = filas});
            }
            catch(ArgumentException excepcionArgumento)
            {
                _logger.LogWarning("ERROR DE VALIDACION - Peticion rechazada - tabla: {Tabla}, Error: {Mensaje}",tabla, excepcionArgumento.Message);
                return BadRequest(new{estado = 400, mensaje = "Parametros de entrada invalidos.", detalle = excepcionArgumento.Message, tabla = tabla});
            }
        ...
        public async Task<IActionResult> ObtenerPorClaveAsync(string tabla, string nombreClave, string valor, [FromQuery]string? esquema = null)
        {
             try 
            { 
          ...
          private object? ConvertirJsonElement(JsonElement elemento)
        {
            return elemento.ValueKind switch
            {
                ...
                JsonValueKind.Object => elemento.GetRawText(), // O se puede convertir a un diccionario anidado si se desea
                JsonValueKind.Array => elemento.GetRawText(),  // O se puede convertir a una lista de objetos si se desea
                _ => elemento.ToString() // Fallback para otros tipos (ej: JsonValueKind.Undefined)
